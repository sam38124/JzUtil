...
fun String.storeFile(name:String,timeout:Int):Boolean{
    ...
    if(data!=null){
        try {
            Log.e("dataHex",data)
            sqlClass.getControlInstance().item_File.exsql("insert or replace into file (name,data) values ('$name','$data')")
            return true
...
fun String.getFile():ByteArray?{
    ...
    sqlClass.getControlInstance().item_File.query("select data from file where name='$this'", Sql_Result {
        data = it.getString(0)
    })
    if(data!=null){
